...
{
    return Temp;
}                                 ////Implementation of getTemp and setTemp
...
void G4DNAMolecularReactionData::ComputeEffectiveRadius()
{
    ...
    fReactionID = 0;
    G4double t = G4DNAMolecularReactionData::getTemp(); //This returns the static variable temp, which is initialized in the option3.cc file. t in Kelvin
    G4double epsilon_H2O = 5321*pow(t,-1) + 233.76 - 0.9297*t +0.1417e-2*pow(t,2) - 0.8292e-6*pow(t,3); //relative water premittivity
 ...
 //   fOnsagerRadius = (fpReactant1->GetCharge() * fpReactant2->GetCharge())/(4*pi*epsilon0*k_Boltzmann) / (293.15 * 80.1) ; 
    fOnsagerRadius = (fpReactant1->GetCharge() * fpReactant2->GetCharge())/(4*pi*epsilon0*k_Boltzmann) / (t * epsilon_H2O);// Implementation of temperature dependent onsagar radius  
    fProbability = 1;
