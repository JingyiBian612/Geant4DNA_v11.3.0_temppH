...
{
    Temperature = 0;
    pH = 0; //Constructor initializes Temperature and pH with 0, but their value is set by the user when the methods setTemp and setpH is called in G4EmDNAChemistry_option3 constructor
...
void G4DNAChemistryManager::setpH(G4double P)
{
    ...
    pH = P;
}                              //Implementation of methods setTemp and setpH
...
void G4DNAChemistryManager::InitializeMaster()
{
    ...
    }else
    {
      ...
      if (!fSkipReactions)
      {
          ...
          fpUserChemistryList->ConstructReactionTablePhTemp(G4DNAMolecularReactionTable::GetReactionTable(),Temperature,pH);//Call to method ConstructReactionPhTemp.
      }
      else
      {
