...
PhysicsList::PhysicsList(G4double T, G4double P) : G4VModularPhysicsList(), Temperature(T), pH(P) //Implementation of the constructor that takes temperature and pH as input
{
...
//    fEmDNAChemistryList = std::make_unique<G4EmDNAChemistry_option3>();
    fEmDNAChemistryList = std::unique_ptr<G4EmDNAChemistry_option3>(new G4EmDNAChemistry_option3(Temperature, pH));//Initialization of chemistry_option3 with contructor that takes temperature and pH as input
    fEmDNAChemistryList->SetVerboseLevel(verboseLevel);
  }
  else {
